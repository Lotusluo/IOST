...
- (void)viewDidLayoutSubviews {
    ...
    NSLog(@"captureView's frame:%@",NSStringFromCGRect(self.captureView.frame));
    NSLog(@"editView's frame:%@",NSStringFromCGRect(self.editView.frame));
}
...
- (void)action:(HQEditImageActionView *)action didClickButton:(UIButton *)button atIndex:(NSInteger)index {
    if (index == 0) { // 旋转
        NSLog(@"image's frame:%@",NSStringFromCGRect(self.imageView.frame));
//        self.rotateTimes ++;
...
- (void)editView:(HQEditImageEditView *)editView anchorPointIndex:(NSInteger)anchorPointIndex rect:(CGRect)rect {
    NSLog(@"anchorPointIndex:%@",NSStringFromCGRect(rect));
    CGRect imageEditRect = [self.editView convertRect:rect toView:self.imageView];
