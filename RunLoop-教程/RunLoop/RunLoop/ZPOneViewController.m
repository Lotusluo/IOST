...
/**
 ...
 RunLoop的处理逻辑：
 当线程运行的时候：
 1、系统首先会通知Observer，线程即将进入RunLoop；
 2、通知Observer，将要处理Timer；
 3、通知Observer，将要处理Source0（点击事件）；
 4、处理Source0（点击事件）；
 5、判断有没有Source1（其他线程发过来的消息或者操作系统内核发过来的消息），如果有的话就直接执行第9步了，如果没有的话就继续执行第6步；
 6、通知Observer，线程即将休眠；
 7、线程休眠，等待被唤醒。在线程休眠的过程中如果外界有Source0、Timer或者外部手动事件的话，就会继续执行第8步；
 8、通知Observer，线程刚被唤醒；
 9、处理唤醒时收到的消息，然后跳回到第2步（Source1接收的事件最终会交给Source0来处理）；
 10、在发生意外的情况下（超时或者程序被杀死），系统会通知Observer，线程即将退出RunLoop。
 */
...
- (void)test
{
    ...
    [NSTimer scheduledTimerWithTimeInterval:1.0 repeats:YES block:^(NSTimer * _Nonnull timer) {
        NSLog(@"使用NSTimer类时会依赖于当前的RunLoop:%@",NSThread.currentThread);
    }];
}
...
- (void)test3
{
    void (^block)(NSString*) = ^(NSString *a){
        NSLog(@"%@", a);
    };
    
    block(@"使用block函数时会依赖于当前的RunLoop");
}
